...
namespace TorrentRuler.Core.Domain.Actions;
...
/// on <em>torrentruler's own</em> filesystem -- so that path must exist and be writable from
/// inside the torrentruler container (mount a volume for it), NOT from qBittorrent. This is the
...
{
    /// <summary>Directory on torrentruler's filesystem to write the .torrent files into. Created if missing.</summary>
